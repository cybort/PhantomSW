...
int XmlParse::parse_xml(void *S_P)
{
    std::string str;
    IHeader itmh;
    STREAM_S *stream = (STREAM_S *)S_P;
    int pkt_cnt = 0;
    ...
    for (; attributeOfStream != NULL; attributeOfStream = attributeOfStream->Next())
    {
        ...
        if (str == "type")
        {
            ...
            std::cout << attributeOfStream->Name() << " : " << (stream->streamtype ? "burst" : "continue") << std::endl;
        }
        if (str == "bursts")
        {
    ...
    for (; packetElement != NULL; packetElement = packetElement->NextSiblingElement())
    {
        ...
        for (; contentElement != NULL; contentElement = contentElement->NextSiblingElement())
        {
            ...
            if (contentName == "ITMH")
            {
                ...
                for (; ITMHElement != NULL; ITMHElement = ITMHElement->NextSiblingElement())
                {
                    ...
                    else if (type == 3)
                    {
                        ...
                        {
                            if (atoi(contactValue.c_str()) > 0x1ffff)
                            {
                                std::cout << "flowID error, scope<0-0x1ffff> " << std::endl;
                                return F_FAIL;
                            }
            ...
            else if (contentName == "data")
            {
                ...
                {
                    std::cout << "datastring error, length scope<0-1024*12> " << std::endl;
                    return F_FAIL;
                }

                stream->pktdata.set_payload(contactValue);
                std::string pload;
                pload = stream->pktdata.get_payload();
                std::cout << "data : " << pload << std::endl;
