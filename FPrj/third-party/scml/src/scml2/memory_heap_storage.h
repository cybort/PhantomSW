...
public:
  ...
  static memory_storage_base* create_storage(toplevel_memory_base& m, const std::string& filename);

private:
  toplevel_memory_base& mMemory;
  unsigned char* mStorage;
  bool m_b_is_storage_map;
  unsigned long long m_ull_storage_size;
  int file_descriptor;
};
