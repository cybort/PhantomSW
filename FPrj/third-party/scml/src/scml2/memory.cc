...
memory(const std::string& name,
  ...
  mFastAccess()
{
  setup(memory_storage_creator::get_instance().create_storage(*this,filename), false);
}
...
memory(const std::string& name,
  ...
  mFastAccess()
{
  setup(memory_storage_creator::get_instance().create_storage(*this), true);
}
...
set_symbolic_name(const DT& value, const std::string& name)
{
  ...
  to_char_array<DT>(value, &tmp[0], sizeOf<DT>(), 0);
