 ...
 * @LastEditors: f21538
 * @LastEditTime: 2020-12-18 10:36:59
 */
#ifndef _RW_LOCK_H_
#define _RW_LOCK_H_
...
class RWLock /*Write first, means frequently/long-helding write op will starve readers. Should use for lots of reading
                and a little writing. If your model/scenario can change, forget rwlock and just use mutex since rwlock
                costs more.*/
{
public:
    RWLock() = default;
...
{
public:
    explicit unique_writeguard(_RWLockable & rw_lockable) : rw_lockable_(rw_lockable) { rw_lockable_.lock_write(); }
...
{
public:
    explicit unique_readguard(_RWLockable & rw_lockable) : rw_lockable_(rw_lockable) { rw_lockable_.lock_read(); }
